...
export async function registerUser(
  ...
  if (!platformApiToken) {
    throw new Error('Contact creation requires CHATWOOT_PLATFORM_API_TOKEN to be configured. Please contact your administrator.');
  }
  
  console.log('Using API token:', platformApiToken.substring(0, 10) + '...');
  console.log('Account ID:', CHATWOOT_ACCOUNT_ID);
  console.log('Creating contact at:', `${CHATWOOT_BASE_URL}/api/v1/accounts/${CHATWOOT_ACCOUNT_ID}/contacts`);
