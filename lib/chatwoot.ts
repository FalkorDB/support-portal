...
import { AuthResponse, ConversationsResponse, MessagesResponse, Message } from '@/types';
...
export async function authenticateUser(email: string, password: string): Promise<AuthResponse> {
  ...
  if (!accessToken) {
    ...
    throw new Error('No access token received from Chatwoot. Make sure you\'re logging in with an agent/admin account.');
  }
 ...
 */
async function authenticateContact(email: string, password: string): Promise<AuthResponse> {
...
export async function fetchConversations(
  ...
  console.log('Auth headers:', { accessToken: accessToken ? 'present' : 'missing' });
  ...
  if (!response.ok) {
    ...
    console.error('Check that the account ID is correct and the user has access');
